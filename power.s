 ...
 #Everything in main is stored in registers, so the data section doesn't have anything.

 # %esp is "top of the stack"
 # %ebp pointer to save position inside stack


.section .data
 #Nothing here, just including out of habit/for posterity
...
_start:
 ...
 popl %ebx        #The second answer is already in %eax.  We saved the first
                  #answer onto the stack, so now we just pop it into %ebx

 addl %eax, %ebx   #add them together. result is in %ebx
